...
void HAL_PCD_SOFCallback(PCD_HandleTypeDef *hpcd) {
    ...
    if (cdc == NULL || !cdc->dev_is_connected || cdc->dbg_mode_enabled ) {
    ...
    if (cdc->tx_buf_ptr_out_shadow != cdc->tx_buf_ptr_in || cdc->tx_need_empty_packet) {
        ...
        if (USBD_CDC_TransmitPacket(cdc->usbd, buffsize, &cdc->tx_buf[buffptr]) == USBD_OK) {
            ...
            cdc->tx_need_empty_packet = (buffsize > 0 && buffsize % CDC_DATA_FS_MAX_PACKET_SIZE == 0
                    && cdc->tx_buf_ptr_out_shadow == cdc->tx_buf_ptr_in);
        }
    }
}
...
static void send_packet(USBD_HandleTypeDef *pdev) {
    ...
    USBD_CDC_TransmitPacket(cdc->usbd, bytes, cdc->dbg_xfer_buffer);
}
    ...
    if (cdc->dbg_mode_enabled == 1) {
        if (cdc->dbg_xfer_length) {
            ...
            if (cdc->dbg_last_packet == CDC_DATA_FS_MAX_PACKET_SIZE) {
                cdc->dbg_last_packet = 0;
                USBD_CDC_TransmitPacket(cdc->usbd, 0, cdc->dbg_xfer_buffer);
            }
        }
    } 
    return USBD_OK;
